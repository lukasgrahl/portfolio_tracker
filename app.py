...
with tab2:
    ...
    mse = mean_squared_error(data['value'], modelfit.fittedvalues)
    ...
    # Display confusion matrix and ROC score
    ...
    ax3.plot(data.index, np.zeros(len(data)), linestyle='--', color='grey', alpha=0.5)
    ax3.plot(data.index, (data['value'] - modelfit.fittedvalues)**2, label='MSE')
    ax3.set_xlabel('Date')
    ax3.set_ylabel('MSE')
    ax3.set_title('ARIMA Model MSE vs. Original Data')
    ax3.legend()
    st.pyplot(fig3)
